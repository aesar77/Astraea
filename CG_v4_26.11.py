...
def import_test_curveset(crvnum, band, bining):
    ...
    for i in range(0, crvnum):
        ...
        duratran = get_curve["duratrantotl"]
...
band = bands[5]
tr = import_test_curveset(25, band[0], bining)
curves = tr 
i=0
...
for c in curves:
    ...
    ortime = c.time
    time = c.binnedtime
    if len(c.listepoch) !=0:
        epoch = extraxt_transits(c)
    else:
        epoch = [[]]
    ...
    if np.size(widths) == 0 or widths[0] < 2:
        ...
        if len(epoch[0]) !=0:
            c.set_correspondance( -(abs(pairs[0] - c.epoch[0][0]) + abs(pairs[1] - c.epoch[0][1])))
        ...
        plt.errorbar(time/1000,curve,yerr = error, marker="o", linewidth=0,elinewidth=0.8, mfc="black",mec="black", ecolor="black", capsize=3)
        ...
        plt.plot(time/1000, orcurve, linewidth=6, color ="red")
        plt.axvline(time[pairs[0]]/1000, ymin=0,ymax=max(curve), linestyle="--", color="g")
        if len(epoch[0]) !=0:
            plt.axvline(c.epoch[0][0]/1000, ymin=0,ymax=max(curve), linestyle="--", color="r")
        plt.legend(["Simulated Curve without noise","Detected Transit Interval","Simulated Transit Interval","Simulated Curve","Obs 13814"])  
        plt.axvline(time[pairs[1]]/1000, ymin=0,ymax=max(curve), linestyle="--", color="g")
        if len(epoch[0]) !=0:
            plt.axvline(c.epoch[0][1]/1000, ymin=0,ymax=max(curve), linestyle="--", color="r")
