...
plt.title("Beta: a, b=" + str(pars))   
rrat = sc.stats.beta.rvs(*pars, size=100)
plt.hist(rrat)
plt.show()
...
def import_test_curveset(crvnum, band, bining):
    ...
    for i in range(0, crvnum):
        ...
        rratcomp = [sc.stats.beta.rvs(*pars, size=1)][0] #rplanet/rstar
        cosicomp = [0]
        pericomp = [10 + sc.stats.beta.rvs(*pars, size=1)*100][0]#scipy ranfom distributions check.
        epoccomp = [lenT/3 + np.random.rand()*lenT/3]
        rsmacomp =  [sc.stats.beta.rvs(*pars, size=1)][0]
...
for c in curves:
    ...
    if np.size(widths) == 0 or widths[0] < 2:
        ...
        curves.remove(curves[c.crvnum-1])
        #widths.remove(width)
...
plt.suptitle("Recall with respect to rflxmodl parameters,  Beta a,b= " + str(pars))
plt.subplot(2,2,1)
...
plt.suptitle("Recall / Parameters,  Beta a,b= " + str(pars))
plt.subplot(4,1,1)
