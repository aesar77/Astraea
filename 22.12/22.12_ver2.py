...
#This version uses directly the paramaters taken from the model sys
...
from ModelSystems_v2 import import_system, rsmacomp_arr, pericomp_arr, rratcomp_arr
...
def import_test_curveset(crvnum, band, bining):
    ...
    for i in range(0, crvnum):
        ...
        rratcomp = [np.random.choice(rratcomp_arr,1)[0]] #rplanet/rstar
        cosicomp = [0]
        pericomp = [np.random.choice(pericomp_arr,1)[0]] #scipy ranfom distributions check.
        epoccomp = [lenT/3 + np.random.rand()*lenT/3]
        rsmacomp = [np.random.choice(rsmacomp_arr,1)[0]] 
        
        rratcomps.append(rratcomp[0])
        pericomps.append(pericomp[0])
        epoccomps.append(epoccomp)
        rsmacomps.append(rsmacomp[0])
        ...
        crv_set.append(Test_Curves(rratcomp[0], cosicomp[0], pericomp[0], time, binnedtime, epoccomp[0], rsmacomp[0], crvnum, original_binned, noisy_curve_norm, noisy_error_norm,transit, listepoch, duratran))    
...
plt.rcParams["figure.figsize"] = (30,15)
plt.subplot(2,2,1)
plt.hist([np.array([x.rratcomp for x in curves if x.result == True]), np.array([x.rratcomp for x in curves if x.result == False])])
plt.title("rratcomp")
plt.legend(["Curves Detected", "Curves Undetected"])
plt.subplot(2,2,2)
plt.hist([np.array([x.pericomp for x in curves if x.result == True]), np.array([x.pericomp for x in curves if x.result == False])])
plt.title("pericomp")
plt.legend(["Curves Detected", "Curves Undetected"])
plt.subplot(2,2,3)
plt.hist([np.array([x.rsmacomp for x in curves if x.result == True]), np.array([x.rsmacomp for x in curves if x.result == False])])
plt.title("rsmacomp")
plt.legend(["Curves Detected", "Curves Undetected"])
plt.subplot(2,2,4)
plt.hist([np.array([x.epoccomp/len(x.time) for x in curves if x.result == True]), np.array([x.epoccomp/len(x.time) for x in curves if x.result == False])])
plt.title("epoccomp")
plt.legend(["Curves Detected", "Curves Undetected"])
plt.show()
...
plt.subplot(2,2,1)
rrath = np.histogram(np.array([x.rratcomp for x in curves if x.result == True]))[0]
rrathf = np.histogram(np.array([x.rratcomp for x in curves if x.result == False]))[0]
plt.scatter(rrath, rrath/(rrath+rrathf),s=100)
plt.title("rratcomp")
plt.subplot(2,2,2)
perih = np.histogram(np.array([x.pericomp for x in curves if x.result == True]))[0]
perihf = np.histogram(np.array([x.pericomp for x in curves if x.result == False]))[0]
plt.scatter(perih, perih/(perih+perihf),s=100)
plt.title("pericomp")

plt.subplot(2,2,3)
rsmah = np.histogram(np.array([x.rsmacomp for x in curves if x.result == True]))[0]
rsmahf = np.histogram(np.array([x.rsmacomp for x in curves if x.result == False]))[0]
plt.scatter(rsmah, rsmah/(rsmah+rsmahf),s=100)
plt.title("rsmacomp")

plt.subplot(2,2,4)
plt.title("epoccomp")
epoch = np.histogram(np.array([x.epoccomp for x in curves if x.result == True]))[0]
epochf = np.histogram(np.array([x.epoccomp for x in curves if x.result == False]))[0]
plt.scatter(epochf, epoch/(epoch+epochf),s=100)
plt.show()
