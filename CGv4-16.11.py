...
#This version uses light curve templates generated by rluxtranmodl function, and adds poisson noise.
 
from ephesus import retr_rflxtranmodl
import numpy as np
import scipy as sc
from os import listdir, walk, makedirs, getcwd, chdir
from os.path import exists, dirname, exists, realpath
from sklearn.metrics import mean_squared_error
from ModelSystems import import_system
...
# This part reads previously extracted light curves of a source. M51 OBsID in this particular case.
...
class Test_Curves():
    def __init__(self,rratcomp,cosicomp, pericomp, time, binnedtime, epoccomp,rsmacomp,crvnum, temp_curve, noisy_curve_norm, noisy_error_norm,transit):
    ...
    def set_parameters(self, dip_mean, dip_extracted_mean, nb_pairs, dip_center, match, width, conditions, params, corr):
...
def import_test_curveset(crvnum, band, bining):
    ...
    for i in range(0, crvnum):
        ...
        rratcomp = [sc.stats.powerlaw.rvs(3, loc =0, scale=0.4, size=1)[0]**(-1) /10] #rplanet/rstar
        cosicomp = [0]
        pericomp = [sc.stats.powerlaw.rvs(3, loc =0, scale=0.3, size=1)[0]**(-1)] #scipy ranfom distributions check.
        epoccomp = [np.random.rand()*len(time)]
        rsmacomp =  [sc.stats.powerlaw.rvs(3, loc =0, scale=1, size=1)[0]**(-1) /10]
        ...
        crv_set.append(Test_Curves(rratcomp, cosicomp, pericomp, time, binnedtime, epoccomp, rsmacomp, crvnum, original_binned, noisy_curve_norm, noisy_error_norm,transit))    
...
band = bands[5]
tr = import_test_curveset(50, band[0], bining)
curves = tr 
i=0
...
for c in curves:
    ...
    if np.size(widths) == 0 or widths[0] < 2:
        print("no suitable minima for a dip candidate.")
        curves.remove(c)
        widths.remove(width)
        nb_pairs.remove([dip - j , dip + i])
        ...
        corr = -(abs(pairs[0]-np.where(orcurve != 1)[0][0]) + abs(pairs[1]-np.where(orcurve != 1)[0][-1]))
        c.set_parameters(dip_mean, dip_extracted_mean, nb_pairs, dip_center, match, max(widths)*bining, conditions, local_params, corr)
        ...
        #c.set_info("Expected dip(s) at: " + str([ortime[np.where(orcurve == x)] for  x in orcurve if x - min(orcurve)<0.001 ]) )
        ...
        if False not in conditions:
            ...
            c.set_info("Possible dip between :" + str([time[pairs[0]], time[pairs[1]]]) + "\n")
            c.set_info("Possible dip at: " + str(c.dipCenter))
            c.set_result(True)
        ...
        plt.suptitle("No " + str(c.crvnum) + ",  Result=" + str(c.result) + ",  Bin Size="+str(bining) + ',  RMS=' + str(rms))
        plt.subplot(2,1,1).set_title("Simulated LC with Poisson Noise")
        plt.ylabel("Counts per bin")
        plt.errorbar(np.linspace(0, len(time), 179), Obs_13814R[0], yerr = Obs_13814R[1])
        plt.errorbar(time,curve,yerr = error, marker="o", linewidth=0,elinewidth=0.8, color="red", mfc="black",mec="black", ecolor="green", capsize=3)
        plt.legend(["M51 Obs 13814", "Noise added curve"])        
        ...
        plt.axvline(time[pairs[0]], ymin=0,ymax=max(curve), linestyle="--", color="r")
        plt.axvline(time[pairs[1]], ymin=0,ymax=max(curve), linestyle="--", color="r")
        plt.subplot(2,1,2).set_title("Simulated LC")
        plt.plot(time, orcurve)
        plt.ylabel("Counts per bin")
        plt.xlabel("Time [ks]")
        plt.show()
...
print("Expected dip number :", len(tr))
...
plt.legend(["Curves Detected", "Curves Undetected"])
plt.subplot(2,2,4)
plt.hist([np.array([x.epoccomp[0] for x in curves if x.result == True]), np.array([x.epoccomp[0] for x in curves if x.result == False])])
plt.title("epoccomp")
plt.legend(["Curves Detected", "Curves Undetected"])
plt.show()
